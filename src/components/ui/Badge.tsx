  ...
  return (
    <div className="relative" ref={badgeRef}>
      ...
      <div
        ...
        {...handleMouseEvents}
      >
        <div className={`w-28 sm:w-32 h-28 sm:h-32 rounded-full border-2 flex flex-col items-center justify-center p-3 sm:p-4 transition-all duration-150 hover:scale-105 active:scale-95 touch-none sm:touch-auto group relative overflow-hidden ring-2 ring-opacity-60 ${certification.featured ? 'ring-orange-500' : 'ring-[var(--accent)]'
          }`}
          style={{
      ...
      {showTooltip && (
        <>
          ...
          <div
            ref={tooltipRef}
            className={`absolute bottom-full mb-4 z-20 animate-fadeIn ${tooltipPosition === 'left' ? 'left-0' :
                tooltipPosition === 'right' ? 'right-0' :
                  'left-1/2 -translate-x-1/2'
              }`}
          >
            <div className={`backdrop-blur-xl backdrop-saturate-150 rounded-lg p-3 sm:p-4 shadow-2xl border max-w-xs w-[250px] sm:w-64 ring-2 ring-opacity-60 ${certification.featured ? 'ring-orange-500' : 'ring-[var(--accent)]'
              }`}
              ...
              {/* Badge Header */}
              <div className={`flex items-center gap-2 mb-3 pb-2 border-b border-opacity-30 ${certification.featured ? 'border-orange-500' : 'border-[var(--accent)]'
                }`}>
                ...
                <span className={`text-xs font-bold tracking-wide ${certification.featured ? 'text-yellow-400' : ''
                  }`} style={{
              ...
              <div className={`text-sm font-bold mb-2 leading-snug tracking-wide ${certification.featured ? 'text-yellow-400' : ''
                }`} style={{
                  color: certification.featured ? '#f59e0b' : 'var(--text-primary)',
                  textShadow: certification.featured
              ...
              {(certification.link || certification.imagePath || certification.pdfPath) && (
                <button
                  ...
                  className={`group/btn relative flex items-center gap-2 mt-3 text-xs px-4 py-2 rounded-md transition-all duration-150 hover:scale-102 w-full justify-center overflow-hidden ${certification.featured ? 'font-semibold' : ''
                    }`}
                  style={{
