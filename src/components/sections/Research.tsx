        ...
        <div className="space-y-8">
          {research.map((paper, index) => {
            ...
            return (
              <div
                key={paper.id}
                className={`group relative overflow-hidden rounded-xl ${smoothTransition} ${cardVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-6 scale-[0.96]'
              ...
              >
                ...
                <div className="relative p-4 sm:p-6 md:p-8 lg:p-12 z-10">
                  <div className="grid lg:grid-cols-3 gap-6 sm:gap-8 items-start">
                      ...
                      <div className="rounded-lg border-2 overflow-hidden"
                        ...
                        <div className="flex border-b" style={{ borderColor: 'var(--border)' }}>
                          <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium transition-all duration-300 ${!showPaperPreview[paper.id] ? 'border-b-2' : ''
                              }`}
                            style={{
                          ...
                          <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium transition-all duration-300 flex items-center justify-center gap-2 ${showPaperPreview[paper.id] ? 'border-b-2' : ''
                              }`}
                            style={{
                              borderColor: showPaperPreview[paper.id] ? 'var(--accent)' : 'transparent',
                              color: showPaperPreview[paper.id] ? 'var(--accent)' : 'var(--text-secondary)',
                        ...
                        <div className="relative overflow-hidden">
                          {/* Abstract Content */}
                          <div className={`transform transition-all duration-[800ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${!showPaperPreview[paper.id]
                    ...
                    {/* Visual Element */}
                    <div className={`lg:col-span-1 flex flex-col items-center justify-center gap-6 h-full transition-all duration-800 delay-1200 ${isVisible ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-90 rotate-6'
                      }`}>
                      <div className="relative group/trophy">
                        ...
                        <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full transition-all duration-500 ${isVisible ? 'animate-ping opacity-70' : 'opacity-0'
                          }`}
                          style={{
                            backgroundColor: 'var(--accent)',
                            animation: isVisible ? 'ping 2s cubic-bezier(0, 0, 0.2, 1) infinite' : 'none'
                          }}></div>
                        <div className={`absolute -bottom-1 -left-1 w-3 h-3 rounded-full transition-all duration-500 ${isVisible ? 'animate-ping opacity-70' : 'opacity-0'
                          }`}
                          style={{
                            backgroundColor: 'var(--accent)',
                            animation: isVisible ? 'ping 2s cubic-bezier(0, 0, 0.2, 1) infinite 1s' : 'none'
                          }}></div>
                        <div className={`absolute -top-2 -left-2 w-2 h-2 rounded-full transition-all duration-500 ${isVisible ? 'animate-ping opacity-50' : 'opacity-0'
                          }`}
                          style={{
                            backgroundColor: 'var(--accent)',
                            animation: isVisible ? 'ping 2s cubic-bezier(0, 0, 0.2, 1) infinite 0.5s' : 'none'
                          }}></div>
                        <div className={`absolute -bottom-2 -right-2 w-2 h-2 rounded-full transition-all duration-500 ${isVisible ? 'animate-ping opacity-50' : 'opacity-0'
                          }`}
                          style={{
                            backgroundColor: 'var(--accent)',
                      ...
                      {/* Badges Container */}
                      <div className={`flex flex-col gap-3 items-center w-full pt-4 transition-all duration-800 delay-1400 ${isVisible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-90 translate-y-4'
                        }`}>
                        {/* Conference Badge */}
                        <div
